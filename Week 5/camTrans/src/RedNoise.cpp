...
void drawPointCloud(DrawingWindow& window, std::string filename, glm::vec3* camPos, glm::mat3* cameraOrientation, bool orbit) {
	window.clearPixels();
	...
	glm::mat3 camOrientation = *cameraOrientation;
...
void handleEvent(SDL_Event event, DrawingWindow &window, glm::vec3 *cameraPosition, glm::mat3* cameraOrientation, bool* orbit) {
...
int main(int argc, char *argv[]) {
	...
	while (true) {
		...
		if (window.pollForInputEvents(event)) handleEvent(event, window, &cameraPosition, &cameraOrientation, &orbit);
		drawPointCloud(window, "cornell-box.obj", &cameraPosition, &cameraOrientation, orbit);
